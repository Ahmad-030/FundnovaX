...
class _ExpenseScreenState extends State<ExpenseScreen> with SingleTickerProviderStateMixin {
  ...
  List<ExpenseModel> get _filtered {
    ...
    return _expenses.where((e) {
      if (_filter == 'Daily') return e.date.day == now.day && e.date.month == now.month && e.date.year == now.year;
      if (_filter == 'Weekly') return e.date.isAfter(now.subtract(const Duration(days: 7)));
      if (_filter == 'Monthly') return e.date.month == now.month && e.date.year == now.year;
      return true;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: NestedScrollView(
        headerSliverBuilder: (_, __) => [SliverToBoxAdapter(child: _buildHeader(isDark))],
      ...
      child: Row(
        children: _filters.map((f) {
          ...
          return Padding(
            ...
            child: ChoiceChip(
              label: Text(f, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600,
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        SizedBox(height: 160, child: TabBarView(
          controller: _tabController,
          children: [
            ExpensePieChart(data: _categoryData, colors: [AppTheme.food, AppTheme.transport, AppTheme.shopping, AppTheme.bills, AppTheme.other]),
      ...
      return [
        Container(
          ...
          child: Center(child: Column(children: [
            ...
            const SizedBox(height: 10),
            Text('No transactions found', style: GoogleFonts.poppins(color: Colors.grey, fontSize: 13)),
          ])),
        )
      ];
    }
    return list.map((e) {
      ...
      return Dismissible(
        ...
        background: Container(
          ...
          decoration: BoxDecoration(color: AppTheme.error.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
        ...
        confirmDismiss: (_) async {
          return await showDialog<bool>(
            ...
            builder: (_) => AlertDialog(
              ...
              content: Text('Remove "${e.title}"?', style: GoogleFonts.poppins(fontSize: 13)),
              actions: [
        ...
        onDismissed: (_) async {
          ...
          await _save();
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('"${e.title}" deleted'), duration: const Duration(seconds: 2)));
        },
        child: Container(
          margin: const EdgeInsets.only(bottom: 8),
          padding: const EdgeInsets.all(14),
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('${e.category.label} • ${DateFormat('MMM d, yyyy').format(e.date)}',
                  style: GoogleFonts.poppins(fontSize: 11, color: Colors.grey)),
            ])),
            Text('${isIncome ? '+' : '-'}\$${e.amount.toStringAsFixed(2)}',
        ...
        builder: (ctx, setModal) {
          ...
          return Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 16),
              Row(children: [
                Expanded(child: _typeBtn('Income', TransactionType.income, type, AppTheme.success, () => setModal(() => type = TransactionType.income))),
                const SizedBox(width: 12),
                Expanded(child: _typeBtn('Expense', TransactionType.expense, type, AppTheme.accent, () => setModal(() => type = TransactionType.expense))),
              ]),
              const SizedBox(height: 14),
              TextField(controller: titleCtrl, decoration: const InputDecoration(hintText: 'Title', prefixIcon: Icon(Icons.edit_outlined))),
              const SizedBox(height: 10),
              TextField(controller: amountCtrl, keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  decoration: const InputDecoration(hintText: 'Amount', prefixText: '\$ ', prefixIcon: Icon(Icons.attach_money))),
              const SizedBox(height: 10),
              DropdownButtonFormField<ExpenseCategory>(
                value: category,
                decoration: InputDecoration(prefixIcon: Padding(padding: const EdgeInsets.all(12), child: Text(category.icon, style: const TextStyle(fontSize: 18)))),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(children: [
                    ...
                    const SizedBox(width: 12),
                    Text(DateFormat('MMM d, yyyy').format(selectedDate), style: GoogleFonts.poppins(fontSize: 13)),
                  ]),
                ),
              ),
              const SizedBox(height: 8),
              Row(children: [
                Switch(value: recurring, onChanged: (v) => setModal(() => recurring = v), activeColor: AppTheme.primary),
                Text('Recurring expense', style: GoogleFonts.poppins(fontSize: 13)),
              ]),
              const SizedBox(height: 16),
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    final entry = ExpenseModel(
                      id: const Uuid().v4(), title: title, amount: amount,
                      category: category, type: type, date: selectedDate, isRecurring: recurring,
                    );
